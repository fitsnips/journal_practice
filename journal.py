...
def menu():
    user_choice = input(
        "Would you like to [A]dd, [L]ist, [R]emove, [P]rint, [U]pdate journal entries or E[x]it: " ).lower().strip()
    return user_choice
...
def entry_list(data):
    ...
    table_entry = PrettyTable( ['Index', 'Title', 'Date Added'] )
    table_entry.align['Index'] = 'l'
    table_entry.align['Title'] = 'l'
    table_entry.align['Date Added'] = 'l'


    for idx, entry in enumerate( data ):
        table_entry.add_row( [str( idx + 1 ), str( entry['title'] ), str( entry['date'] )] )
